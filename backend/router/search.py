...
from backend.resources.crawler import search_title, Chapters
...
async def get_book_chapters(book_id: str) -> BooksResponseModel:
    """取得書本章節"""
    chapters = Chapters(book_id=book_id)
    await chapters.crawl()
    books = chapters.results
    print(len(books))
    return {"books": books}
